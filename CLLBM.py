...
#The Book class: Represents a single book with details
...
class Book:
    ...
    def from_dict(data):
        return Book(**data)
#The LibraryManager class: Manages the collection of books and user commands
class LibraryManager:
    def __init__(self, filename='books.txt'):
        self.filename = filename
        self.books = []
        self.load_data()
#Adding a new book interactively
    def add_book(self):
        title = input("Title: ")
        author = input("Author: ")
        try:
...
#Searching books by title, author, or year
...
#Helper function to pick a book by its list number
...
#Saving book data to a file
    def save_data(self):
        with open(self.filename, 'w') as f:
            json.dump([book.to_dict() for book in self.books], f, indent=2)
#Loading book data from a file if it exists
    def load_data(self):
